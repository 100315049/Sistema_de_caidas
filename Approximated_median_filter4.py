...
# '..\Prog\Videos_TFG\persona_fondo_azul.wmv'
path = 'persona_fondo_azul.wmv'
path2 = 'Gata.wmv'
path3 = 'Caida JerseyBlanco.wmv'
# C:\Users\sergio\Desktop\TFG\Prog\Videos_TFG\UR Fall Detection Dataset
path4 = 'fall-05-cam0_MOD.avi'
path5 = 'adl-01-cam0_MOD.avi'
...
compNum = 0.4    # Indica cuanto se añade o se resta cada frame
...
if ret == True:
    ...
    backgroundGris = cv2.GaussianBlur(backgroundGris, (9, 9), 0)
    # Para poder restar numeros decimales
    backgroundFloat = np.array(backgroundGris,float)
    ...
    while(1):
        ...
        ero = cv2.erode(foreground, np.ones((3,3), np.uint8), iterations=1)
        dil = cv2.dilate(ero, np.ones((9,9), np.uint8), iterations=1)        
        
        opening = cv2.morphologyEx(foreground, cv2.MORPH_OPEN, kernel)
        closing = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel)
 
        # Copiamos el umbral para detectar los contornos
        contornosimg = dil.copy()
        ...
        cv2.add(backgroundFloat, compNum, backgroundFloat, maskCompG) # (No se produce desbordamiento)
        # Si el pixel del frame es menor que el background se resta 1 al background
        cv2.subtract(backgroundFloat, compNum, backgroundFloat, mask = maskCompL) # (No se produce desbordamiento)
        backgroundGris = np.array(backgroundFloat, np.uint8)
